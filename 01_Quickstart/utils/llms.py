...
def initialize_llm(llm_type: str = DEFAULT_LLM_TYPE) -> tuple[ChatOpenAI, OpenAIEmbeddings]:
    ...
    try:
        ...
        # 对需要密钥的 provider 做显式校验，避免延迟到请求阶段才失败
        if llm_type in {"oneapi", "qwen"} and not config["api_key"]:
            raise ValueError(f"{llm_type} 的 API key 未配置，请设置对应环境变量")
...
def get_llm(llm_type: str = DEFAULT_LLM_TYPE) -> tuple[ChatOpenAI, OpenAIEmbeddings]:
    ...
    Returns:
        tuple[ChatOpenAI, OpenAIEmbeddings]: LLM与Embedding实例
    """
    try:
